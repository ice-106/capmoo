...
const openSearchDrawer = useCallback(() => {
  ...
  }), [searchParams]); // Only depends on searchParams object
  ...
  useEffect(() => {
    ...
    const mockResults = [
      { imgUrl: defaultImageUrl, text: "Result 1" + (queryParams.q ? ` for "${queryParams.q}"` : ""), onClickUrl: "/" },
      { imgUrl: defaultImageUrl, text: "Result 2" + (queryParams.locations.length ? ` in ${queryParams.locations.join(', ')}` : ""), onClickUrl: "/" },
      { imgUrl: defaultImageUrl, text: "Result 3" + (queryParams.minPrice ? ` min price ${queryParams.minPrice}` : ""), onClickUrl: "/" },
      { imgUrl: defaultImageUrl, text: "Result 4" + (queryParams.maxPrice ? ` max price ${queryParams.maxPrice}` : ""), onClickUrl: "/" },
      { imgUrl: defaultImageUrl, text: "Result 5" + (queryParams.categories.length ? ` (${queryParams.categories.join(', ')})` : ""), onClickUrl: "/" },
  ...
  const handleSearch = useCallback((formValues?: SearchFormValues) => {
    if (formValues) {
  ...
  return (
    ...
    onTouchEnd={swipeHandlers.onTouchEnd} />
      ...
      <div className="mb-6">
        ...
        <div className="flex w-full mt-4 text-darkgrey items-start">  
          ...
          <div className="flex flex-wrap gap-2 justify-between w-full">
            ...
            {(queryParams.minPrice || queryParams.maxPrice) && (
              <FilterTag onClick={openSearchDrawer}>
                Price: {queryParams.minPrice || '0'} - {queryParams.maxPrice || 'Any'} Baht
              </FilterTag>            
            )}
            ...
            { queryParams.categories.length == 0 && queryParams.locations.length == 0 && queryParams.maxPrice == null && queryParams.minPrice == null &&
