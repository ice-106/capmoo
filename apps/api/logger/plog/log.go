...
func (h *Handler) Handle(ctx context.Context, r slog.Record) error {
	...
	var source string
	sourceData, exist := attrs["source"]
	if sourceMap, ok := sourceData.(map[string]any); exist && ok {
		functionName, ok1 := sourceMap["function"].(string)
		line, ok2 := sourceMap["line"].(float64)
		if ok1 && ok2 {
			source = colorize(darkGray, fmt.Sprintf("[%s:%v]", functionName, line))
			delete(attrs, "source")
		}
	}
	...
	b = bytes.ReplaceAll(b, []byte(`"err"`), []byte(
		"\033[0m"+colorize(lightRed, `"err"`)+"\033[0m\033[37m",
	))
	b = bytes.ReplaceAll(b, []byte(`"error"`), []byte(
		"\033[0m"+colorize(lightRed, `"error"`)+"\033[0m\033[37m",
	))
